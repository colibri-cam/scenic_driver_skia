...
#
# Create and initialize a directory for building a custom Buildroot sysroot
# for Scenic Driver Skia cross-compilation.
#
# Inputs:
#   $1 = the path to the configuration file (a _defconfig file)
#   $2 = the build directory (optional)
#
# Output:
...
if [[ -z $DEFCONFIG ]]; then
    ...
    echo
    echo "Example:"
    echo "  $0 configs/scenic_skia_aarch64_defconfig"
    exit 1
fi
...
BUILDROOT_STATE_FILE=$BASE_DIR/buildroot-$BUILDROOT_VERSION/.scenic-skia-br-state
BUILDROOT_EXPECTED_STATE_FILE=$BUILD_DIR/.scenic-skia-expected-br-state
"$BASE_DIR/scripts/buildroot-state.sh" $BUILDROOT_VERSION "$BASE_DIR/br_patches" > "$BUILDROOT_EXPECTED_STATE_FILE"
...
create_buildroot_dir() {
    ...
    # Apply buildroot specific patches if any exist
    if [[ -d "$BASE_DIR/br_patches" ]] && ls "$BASE_DIR/br_patches"/*.patch >/dev/null 2>&1; then
...
if [[ ! -e $BUILDROOT_STATE_FILE ]]; then
    ...
    echo "to a change in Buildroot or a change in the patches that get"
...
echo "   2. make"
echo
echo "Or run: ./build.sh $DEFCONFIG"
echo
